...
node* FWTfor(node* arg_node, info* arg_info) {
    ...
    node *loopVar = TBmakeId(STRcpy(varDefSTEName), NULL);
    ...
    node* stepVarDef = TBmakeVardef(FALSE, FALSE, STRcpy(ID_NAME(stepVar)), TY_int, NULL, NULL, NULL, NULL);
    node* finishVarDef = TBmakeVardef(FALSE, FALSE, STRcpy(ID_NAME(finishVar)), TY_int, NULL, NULL, NULL, NULL);
